...
plot_data <- data_long %>% 
  mutate (
    drug = factor (drug, level = c("INH", "RIF", "EMB", "PZA", "STM", "AMK",
                                   ...
                                   "CFZ", "DLM", "PMD", "LZD")),
    result = case_when(result == "fail" ~ 
                         "fail or insufficient sample coverage",
                       #result == "undetermined" ~ "fail",
                       TRUE ~ result),
    result = factor(result, level = c("fail or insufficient sample coverage",
                                      "undetermined", 
                                      "resistant", 
                                      "sensitive")))
```
...
# Aesthetics
plot <- plot +
  scale_fill_manual(values = c("#bdbdbd",#"#f5f5f5", 
  ...
  theme(text = element_text(size = 18),
        ...
        axis.text.x = element_text (angle = 45, hjust = 1),
        legend.position = "bottom",) +
  guides(fill = guide_legend(nrow = 4)) +  # Multiple rows for the legend
  # Remove legend title
