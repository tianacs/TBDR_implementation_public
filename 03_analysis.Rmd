...
combined_table <- demographic_data %>%
  tbl_summary(
    ...
    missing = "always",  # Always show NAs for the variables
...
### Forrest plot for visualisation
...
### Investigating sequencing success rates by preprocessing or site

```{r Differences when preprocessing, include=TRUE}
...
site <- addmargins (table (subset$qc_status, subset$site))

print (site)

pass.SA <- site["pass", "NICD"]
pass.ZM <- site["pass", "CIDRZ"]
total.SA <- site["Sum", "NICD"]
total.ZM <- site["Sum", "CIDRZ"]
```

South Africa had a sequencing success rate of `r pass.SA`/`r total.SA` (`r round (pass.SA/total.SA*100, 2)`%) from sputum

Zambia had a sequencing success rate of `r pass.ZM`/`r total.ZM` (`r round (pass.ZM/total.ZM*100, 2)`%) from sputum
...
South Africa had a sequencing success rate of `r pass.SA`/`r total.SA` (`r round (pass.SA/total.SA*100, 2)`%) from sediment

Zambia had a sequencing success rate of `r pass.ZM`/`r total.ZM` (`r round (pass.ZM/total.ZM*100, 2)`%) from sediment

```{r}
data %>% 
  group_by(is_paired, sample_type) %>% 
  summarise(number_pass = sum(qc_status == "pass"),
            number_total = n(),
            perc_pass = number_pass/number_total*100) %>% 
  arrange (sample_type)
```
...
plot_data <- data_long %>% 
  mutate (
    ...
    result = case_when (qc_status == "fail" ~ "fail",
                              result == "undetermined" ~"fail",
                              qc_status == "pass" ~ result),
    result = factor (result, level = c("fail", "resistant", "sensitive")))
```
...
# Aesthetics
plot <- plot +
  scale_fill_manual(values = c( "#bdbdbd","#DC863B","#5ab4ac")) +
  theme_bw () +
  theme(text = element_text(size = 18),
        ...
        legend.position = "bottom",) +
  # Remove legend title
  labs (fill = "TBDR predicted susceptibility",
        x = "Preprocessing",
        y = "Percentage of samples")

plot

#ggsave("figures/targetplot.png", plot, width = 26.8, height = 24.4, units = "cm")
```
...
create_DTA_counts <- function(data_in) {
  ...
  overall_xpert <- contingency_xpert %>% 
    ...
    mutate(drug = "Xpert® MTB/RIF Ultra (RIF)")
...
dta_df %>% 
  ...
  summarise(tngs_sedi = sum(result.sediment %in% c("resistant", "sensitive")),
            ...
            pDST = sum(dst_result %in% c("resistant", "sensitive")),
...
spoligo %>% 
  ...
  pivot_wider(names_from = site, 
              values_from = count) %>% 
  arrange(desc(lineage), spoligotype) %>% 
  adorn_totals() %>% 
  #write_clip ()
  gt::gt()
